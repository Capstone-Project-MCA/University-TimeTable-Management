      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <header className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4 animate-fade-in-up">
            <div>
                ...
                Select sections to merge. You must select at least 2 sections.
                </p>
            </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="glass p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-visible">
              ...
              <div className="relative" ref={searchRef}>
                <div className="relative" ref={anchorRef}>
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-white/50 dark:bg-slate-900/50 rounded-2xl border-2 border-slate-100 dark:border-slate-800 focus:border-primary/50 focus:ring-4 focus:ring-primary/10 transition-all outline-none text-sm font-medium"
                  />
                    ...
                    <div className="overflow-y-auto py-2 scrollbar-hide">
                      {suggestions.length === 0 ? (
                        ...
                        suggestions.map(c => (
                          ...
                          >
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${colorBg[getColor(c.CourseCode)]}`}>
                              <span className="text-[10px] font-black">{c.CourseCode.substring(0,2)}</span>
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-bold text-slate-700 dark:text-slate-200 truncate">{c.CourseTitle}</p>
                            </div>
                          </button>
                        ))
                      )}
              ...
              <div className="animate-fade-in-up bg-gradient-to-br from-primary to-blue-600 p-6 rounded-3xl text-white shadow-2xl relative overflow-hidden">
                <div className="relative">
                  <div className="flex justify-between items-start mb-4">
                    <span className="bg-white/20 px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase">Active Course</span>
                    <button onClick={handleChangeCourse} className="hover:rotate-90 transition-transform p-1 bg-black/10 rounded-lg">
                      <span className="material-symbols-outlined text-sm">close</span>
                    </button>
                  </div>
                  <h3 className="text-xl font-black">{selectedCourse.CourseTitle}</h3>
                  <p className="text-blue-100 font-bold tracking-widest uppercase text-xs">{selectedCourse.CourseCode}</p>
                </div>
              </div>
            )}
          </div>

          {/* ── Right Side: Sections & ALWAYS VISIBLE Merge Button ── */}
          <div className="lg:col-span-8 flex flex-col h-full min-h-[500px]">
            <div className="flex-1 relative glass rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl p-6 flex flex-col overflow-hidden">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-black text-slate-800 dark:text-slate-100">Sections</h2>
              ...
              <div className="flex-1 overflow-y-auto pr-2 -mr-2 space-y-3 custom-scrollbar">
                {!selectedCourse ? (
                  <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
                    <div className="w-24 h-24 rounded-[40px] bg-slate-100 dark:bg-slate-800 flex items-center justify-center mb-6">
                      <span className="material-symbols-outlined text-5xl text-slate-400">search</span>
                    </div>
                    <h3 className="text-lg font-black text-slate-400">Search Course First</h3>
                    <p className="max-w-[200px] text-xs font-medium mt-2">Search "CSE" in the sidebar to see sections.</p>
                  </div>
                ) : relatedSections.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
                    <h3 className="text-lg font-black text-slate-400">No Sections found</h3>
                  </div>
                ) : (
                    ...
                    {relatedSections.map((s) => {
                      ...
                      return (
                        <div 
                          ...
                          className={`p-5 rounded-2xl border-2 cursor-pointer transition-all ${isSelected ? "bg-primary/5 border-primary" : "bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800"}`}
                        >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4">
                <div className="glass px-6 py-4 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-2xl flex items-center justify-between gap-4 backdrop-blur-xl">
                  <div className="flex items-center gap-4">
        ...
        {/* ── SUCCESS CHIP ── */}
        {lastMerge && (
          <div className="mt-8 bg-emerald-500 text-white p-5 rounded-3xl flex items-center justify-between animate-fade-in-up shadow-xl">
            <div className="flex items-center gap-4">
                <span className="material-symbols-outlined">check_circle</span>
                <div>
                    <h4 className="font-black text-sm uppercase">Merge Complete</h4>
                    <p className="text-xs opacity-80">Reference Code: {lastMerge.mergeCode}</p>
                </div>
            </div>
            <button onClick={() => setLastMerge(null)} className="p-2 hover:bg-white/20 rounded-xl">
